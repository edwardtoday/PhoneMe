 ...
 * <p>
 * 封装了对group的基本操作， 包括group的初始化、保存，删除、修改等。
 * <p>
 * 负责维护group文件。 文件中保存且仅保存组的名字和组成员的id。 文件的第一行为组的名字， 接下来每一行为一个组成员的id， 最后会有一行空行。
 * 如果在读group文件时， 读组名时出现空行则将组名置为默认值， 读组成员时出现非数字的行或空行则忽略， 并继续读接下来的行， 直到读到文件尾。
 */
class Group implements I_Constants, I_Group {
	 ...
	 *             尚未确认影响
	 */
	private Group(String groupName) throws IOException {
		// 找到未使用的id
	 ...
	 *             尚未确认影响
	 */
	private Group(int groupId) throws FileNotFoundException, IOException {
	...
	/**
	 * 通过组名新建一个group，并保存成文件 并给它赋予一个与已知group的id不同的id。 <br>
	 * <strong> 该方法只用于新建组， 要从文件中创建组请使用int参数的方法 </strong></br>
	 * 
	 * @param groupName
	 *            group的名字
	...
	public void save() throws IOException {
		...
		// 写入group文件
		WriteFile wf = new WriteFile(GROUP_PATH + String.valueOf(groupId),
				false);
		wf.write(temp);
		wf.close();
	}
