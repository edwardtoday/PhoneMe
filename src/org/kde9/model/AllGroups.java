 ...
 * <p>
 * 封装了对AllGroups的基本操作， 包括AllGroups的初始化、保存，删除、修改等。
 * <p>
 * 负责维护AllGroups文件。 文件中保存且仅保存所有组的id， 每个id占一行，文件结尾会有一空行。
 * 如果在读ALLGroups文件时出现非数字的行或空行则忽略， 并继续读接下来的行， 直到读到文件尾。
 */
class AllGroups implements I_Constants, I_AllGroups {
	 ...
	 *             尚未确认影响
	 */
	private AllGroups() throws FileNotFoundException, IOException {
	...
	/**
	 * 调用构造方法， 从AllGroups文件中读取信息建立一个AllGroups对象。
	 * <p>
	 * 如果在读ALLGroups文件时出现非数字的行或空行则忽略， 并继续读接下来的行， 直到读到文件尾。
	 * 
	 * @throws FileNotFoundException
	 ...
	 *             尚未确认影响
	 */
	public static I_AllGroups createAllGroups() throws FileNotFoundException,
			IOException {
